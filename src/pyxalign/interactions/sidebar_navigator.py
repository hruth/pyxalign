...
class SidebarNavigator(QWidget):
    def __init__(self):
        ...
        # # Final window settings (uncomment as needed)
        # self.setWindowTitle("QToolBar Navigation Example")
        # self.resize(600, 400)
    ...
    def addPage(self, page_widget: QWidget, title: str, icon: QIcon = None):
        ...
        :param icon: (Optional) A QIcon for the toolbar action.
        """
        # Create a frame to contain the passed-in widget
        page_frame = QFrame()
        page_frame.setFrameShape(QFrame.StyledPanel)
        page_frame.setFrameShadow(QFrame.Raised)

        # Lay out the passed widget inside the frame
        layout = QVBoxLayout(page_frame)
        layout.addWidget(page_widget)

        # Add the frame (instead of the raw widget) to the QStackedWidget
        index = self.stacked_widget.addWidget(page_frame)

        # Create the QAction for this page
        action = QAction(icon if icon else QIcon(), title, self)
        action.setCheckable(True)
        self.action_group.addAction(action)

        # Switch to the new page when this action is triggered
        action.triggered.connect(partial(self.stacked_widget.setCurrentIndex, index))

        # Add the action to the toolbar
        self.toolbar.addAction(action)

        # If it's the first page, mark its action as checked by default
        if self.stacked_widget.count() == 1:
            action.setChecked(True)
...
def main():
    ...
    # Create an instance of SidebarNavigator
    main_widget = SidebarNavigator()
