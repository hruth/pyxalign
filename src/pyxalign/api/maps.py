from typing import Callable, Union
import numpy as np
import cupy as cp

from pyxalign.api.enums import (
    DownsampleType,
    RotationType,
    ShiftType,
    UpsampleType,
    DeviceType,
    MemoryConfig,
    ProcessFunc,
)
import pyxalign.transformations.functions
import pyxalign.gpu_utils as gutils

from pyxalign.api.types import ArrayType


class ShiftProtocol:
    def __call__(self, images: ArrayType, shift: int) -> ArrayType: ...


class RotationProtocol:
    def __call__(self, images: ArrayType, angle: int) -> ArrayType: ...


# Functions with enum inputs
# To do: make a protocol to help with type hints
def get_downsample_func_by_enum(key: DownsampleType) -> Callable:
    return {
        DownsampleType.FFT: pyxalign.transformations.functions.image_downsample_fft,
        DownsampleType.LINEAR: pyxalign.transformations.functions.image_downsample_linear,
        DownsampleType.NEAREST: pyxalign.transformations.functions.image_downsample_nearest,
    }[key]


def get_upsample_func_by_enum(key: UpsampleType) -> Callable:
    return {
        UpsampleType.NEAREST: pyxalign.transformations.functions.image_upsample_nearest,
    }[key]


def get_shift_func_by_enum(key: DownsampleType) -> ShiftProtocol:
    return {
        ShiftType.CIRC: pyxalign.transformations.functions.image_shift_circ,
        ShiftType.FFT: pyxalign.transformations.functions.image_shift_fft,
        ShiftType.LINEAR: pyxalign.transformations.functions.image_shift_linear,
    }[key]


def get_rotation_func_by_enum(key: RotationType) -> RotationProtocol:
    return {
        RotationType.FFT: pyxalign.transformations.functions.image_rotate_fft,
    }[key]


def get_shear_func_by_enum(key: RotationType) -> RotationProtocol:
    return {
        RotationType.FFT: pyxalign.transformations.functions.image_shear_fft,
    }[key]


def get_process_func_by_enum(key: Union[None, ProcessFunc]) -> Callable:
    return {
        ProcessFunc.ANGLE: np.angle,
        ProcessFunc.ABS: np.abs,
        ProcessFunc.NONE: lambda x: x,
        None: lambda x: x,  # bandaid fix, this really shouldn't be in here!
    }[key]


# Functions with enum outputs
def get_memory_config_enum(keep_on_gpu: bool, device_type: DeviceType):
    if keep_on_gpu:
        return MemoryConfig.GPU_ONLY
    elif device_type == DeviceType.GPU:
        return MemoryConfig.MIXED
    else:
        return MemoryConfig.CPU_ONLY
